...
export class AddBlogPost {
    ...
    constructor(page: Page) {
        ...
        this.removeBackgroundButton = page.locator('span[aria-label="Remove background image"]');
    ...
    async checkAddBlogPostPagePlaceholders() {
        const placeholderChecks = [
            { element: this.postTitleTextFiled, expectedText: addBlogPostDict.placeholders.postTitle },
            { element: this.postSummaryTextFiled, expectedText: addBlogPostDict.placeholders.postSummary },
            { element: this.postContentTextFiled, expectedText: addBlogPostDict.placeholders.postContent },
        ];
    ...
    async checkIfFieldsAreEmpty() {  
        for (const check of this.fieldChecks) {
            ...
            if (!isEmpty) {
                ...
                console.error(`Field "${check.fieldName}" is not empty. Current content: "${actualText?.trim()}"`);
            }
    ...
    async checkIfFieldsAreNotEmpty() {
        for (const check of this.fieldChecks) {
            const isEmpty = await checkTextFieldEmptyStatus(check.element);
            if (isEmpty) {
                console.error(`Field "${check.fieldName}" is empty when it should not be.`);
            }
